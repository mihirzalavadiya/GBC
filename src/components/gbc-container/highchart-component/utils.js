...
export const userJourneyData = (userDetails) => {
  ...
  if (
    (kycStatus === 'USER_APPROVED' || kycStatus === 'APPROVED') &&
    billingType === 'PRIVATE_WEALTH'
  ) {
    return {
      title: (
        <>
          Contact your advisor to help you achieve <br /> your target rate of
          returns
        </>
      ),
    ...
    return {
      title: (
        <>
          Complete your KYC process to unlock the full <br /> features of the
...
export const getHeadersRM = () => {
  ...
  const defaultHeaders = {
    ...
    'User-IP':
      window.localStorage.ipAddress === undefined
        ? '0.0.0.0'
        : window.localStorage.ipAddress,
