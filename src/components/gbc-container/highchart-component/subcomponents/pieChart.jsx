...
const PieChart = ({ analytics }) => {
  const { blurLeftSection, setBlurLeftSection, calculateReturns, setCalculateReturns, modalRateofReturn, setModalRateofReturn, rmEditable, userDetails, rmLevel, plateform } = useContext(GBCcontext);
  ...
  const transformedData = assetAllocation && assetAllocation?.map((item) => [item?.assetClass, item?.allocationPercent * 100]);
  ...
  const colorOutput = transformedData
    ? assetAllocation
        ?.map((item) => {
          ...
          const colorObj = colorMapping?.find((obj) => obj?.assetClass === assetClass);
    ...
    () => ({
      ...
      legend: {
        ...
        labelFormatter: function () {
          return `<div class="d-flex"><div>${this.name} -</div> <div>${this.percentage.toFixed(2)}%</div></div>`;
        },
      },
      title: {
        text: ``,
  ...
  return (
    <div className="pie-chart-conatiner">
      ...
      <HighchartsReact highcharts={Highcharts} options={options} allowChartUpdate containerProps={{ style: { height: "100%" } }} />
      {/* </div> */}
      <div id="container"></div>
      <div id="legend-container"></div>
      <div id="containerData" className="containerData" style={plateform ? {} : { width: "100%" }}>
        <div className="chart-percentage">
          {rateOfReturn ? rateOfReturn?.toFixed(1) : 0.0}
          <span className="chart-percent-sign">%</span>
        </div>
        {plateform ? <div className="chart-text">Annual Target Rate of Return</div> : <div className="chart-text">Annual Target Rate of Return</div>}
