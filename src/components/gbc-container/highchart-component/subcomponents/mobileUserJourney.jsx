...
const MobileUserJourney = ({ upgradeAccreditedRequestHandlerForGBC, sendByOkReq, zendeskHandler, analytics }) => {
  const { userDetails, setUserJourneyBtnActionName, answersObj, calculateReturns, plateform, rmLevel, rmEditable, setRmEditable, minimumReturnPercentage, maximumReturnPercentage, rmlevel, apiBaseUrl, landingTime } = useContext(GBCcontext);
    ...
    <>
        ...
        <Row className="mobile-user-journey-container">
          {btn2 && (
            <Col span={24} className="flex row-center">
              <Button
                className={userDetails?.kycStatus === "APPROVED" && rateOfReturn > maximumReturnPercentage && btn2 === "Model Asset Allocation" ? "mobile-user-btn-disable" : "mobile-user-btn-enable"}
          ...
          {btn2 !== "Complete KYC" && btn1 && (
            <Col span={24} className="flex row-center">
              <Button type="link" className="mobile-user-btn-link" onClick={() => btn1Handler(btn1)}>
                {btn1}
              </Button>
            </Col>
          )}
          {btn2 === "Complete KYC" && (
            <Col span={24} className="flex row-center">
              <Button type="link" className="mobile-user-btn-link" onClick={() => btn1Handler("skip-to-dashboard")}>
                Skip to Dashboard
              </Button>
            </Col>
          )}
      ...
      {!plateform && (
        <Row className="mobile-user-journey-container">
          {!rmEditable && (
            <Col span={24} className="flex row-center">
              <Button className="mobile-user-btn-enable" onClick={() => editableHandler()}>
                Edit
              </Button>
            </Col>
          )}
          {rmEditable && (
            <Col span={24} className="flex row-center">
              <Button className="mobile-user-btn-enable" onClick={() => saveChanges()}>
                Save Changes
              </Button>
            </Col>
          )}
