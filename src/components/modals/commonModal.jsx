...
import { calculateTimeDifference, errorMessageData } from "../gbc-container/highchart-component/utils";
...
const CommonModal = ({ analytics, upgradeAccreditedRequestHandlerForGBC }) => {
  ...
  const { userDetails, blurLeftSection, userJourneyBtnActionName, setUserJourneyBtnActionName, plateform, answersObj, calculateReturns, setRmEditable, apiBaseUrl, showPopup, setShowPopup, landingTime, minimumReturnPercentage, maximumReturnPercentage, rmLevel } = useContext(GBCcontext);
  ...
  const handleCancel = () => {
    ...
    if (showPopup && userDetails && userDetails.kycStatus === "APPROVED") {
      saveParams(answersObj, rateOfReturn, modelPortfolioId, showPopup, userDetails, apiBaseUrl, plateform)
        ?.then(() => {
          setIsModalOpen(false);
          setUserJourneyBtnActionName("");
        })
  ...
  const saveHandler = () => {
    setLoading(true);
    saveParams(blurLeftSection ? {} : answersObj, rateOfReturn, modelPortfolioId, showPopup, userDetails, apiBaseUrl, plateform)
      .then(() => {
  ...
  useEffect(() => {
    if (userJourneyBtnActionName === "Talk to Advisor") {
      ...
      setIsModalOpen(true);
    } else if (userJourneyBtnActionName === "Save Changes" || userJourneyBtnActionName === "Complete KYC" || userJourneyBtnActionName === "Model Asset Allocation" || userJourneyBtnActionName === "Skip to dashboard") {
      setIsModalOpen(true);
    } else if (Object.keys(userDetails || {})?.length > 0 && userDetails && userDetails.kycStatus === "APPROVED" && plateform && showPopup) {
      console.log("hello12234");
      setIsModalOpen(true);
    } else if (Object.keys(userDetails || {})?.length === 0 && userJourneyBtnActionName === "Signup") {
  ...
  }, [userJourneyBtnActionName, showPopup]);
  ...
  const accredited_investor = [
    {
      title: "Annual Income in the preceding year of Singapore Dollars 300,000/- (or the equivalent in foreign currency)",
    },
    {
      title: "Total Net Worth of Singapore Dollars 2,000,000/- (or equivalent in foreign currency) with the value of his/her primary residence capped at SGD 1 million",
    },
    {
      title: "Whose financial assets (net of any related liabilities) exceed in value SGD 1 million (or it’s equivalent in a foreign currency)",
    },
  ];
  ...
  const bodyData = (userJourneyBtnActionName) => {
    if (userJourneyBtnActionName === "Talk to Advisor") {
      return (
        <>
          <Row className="font-size-24 line-height-28 bold modal-title-gbc">Request Noted</Row>
          <Row className="width-90 font-size-14 line-height-20 text-999 margin-top-8 modal-subtitle-gbc">Your RM will contact you shortly to understand your requirement & advise you on next steps.</Row>
        </>
      );
    } else if (userJourneyBtnActionName === "Upgrade to Private Wealth") {
      return (
        <>
          <Row className="font-size-24 line-height-28 bold modal-title-gbc">Accredited Investor</Row>
          <Row className="width-90 font-size-14 line-height-20 text-999 margin-top-8 modal-subtitle-gbc">
            Accredited Investors are our customers that meet the following criteria and have opted to be so:
            <p className="font-size-14 line-height-20 text-777 margin-top-30">
              {accredited_investor?.map(({ title }, index) => (
                <Row className="margin-top-24">
                  <Col span={3} className="flex row-center column-center text-1f1f1f text-bold-700">
                    <CheckOutlined />
                  </Col>
          ...
          <Row className="flex row-center">
            <Button type="text" className="btnLink" onClick={upgradeAccreditedRequestHandler}>
              OK
            </Button>
          </Row>
        </>
      ...
      return (
        <>
          <Row className="font-size-24 line-height-28 bold modal-title-gbc">Save Changes?</Row>
          <Row className="width-90 font-size-14 line-height-20 text-999 margin-top-8 modal-subtitle-gbc">Do you wish to save all the changes to the goal based returns caluclator?</Row>
          {!plateform && (
            <Row>
              <span className="rm-save-changes-note space-top">
                <b>Note:</b> These changes will be informed to your client via notification.
              </span>
            </Row>
          )}
          <Row className="margin-top-20 save-modal-btns">
            ...
            <Button className="margin-left-5 btnModalEnable pointer" onClick={saveHandler} loading={loading}>
      ...
      return (
        <>
          <Row className="flex row-center text-center font-size-24 line-height-28 bold">Congratulations!</Row>
      ...
      return (
        <>
          ...
          <Row className="font-size-18 line-height-24 bold margin-top-50">Saving all your inputs.</Row>
  ...
  return (
    <Modal
      ...
      footer={false}
      maskClosable={Object.keys(userDetails || {})?.length === 0 && userJourneyBtnActionName === "Signup" ? false : true}
      closable={Object.keys(userDetails || {})?.length === 0 && userJourneyBtnActionName === "Signup" ? false : true}
      width={userJourneyBtnActionName === "Upgrade to Private Wealth" ? 610 : 594}
    ...
    >
      {userJourneyBtnActionName === "Signup" && (
        <Row className="flex row-center">
          <img src={typeof savingImage === "object" ? savingImage.src : savingImage} height={255.9} width={383} />
        </Row>
      )}
      {showPopup &&
        userDetails &&
        ...
        userJourneyBtnActionName === "Skip to dashboard" && (
          <Row className="flex row-center">
            <img src={typeof trophyAndTarget === "object" ? trophyAndTarget.src : trophyAndTarget} height={261} width={287.9} />
          </Row>
        )}
